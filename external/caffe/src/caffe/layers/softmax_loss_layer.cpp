...
void SoftmaxWithLossLayer<Dtype>::LayerSetUp(
  ...
  weight_by_label_freqs_ =
    this->layer_param_.softmax_loss_param().weight_by_label_freqs();
  ...
  if (weight_by_label_freqs_) {
    ...
    for (int i = 0; i < this->layer_param_.softmax_loss_param().class_weighting_size(); i++) {
        label_count_data[i] = this->layer_param_.softmax_loss_param().class_weighting(i);
    }
  }
}
